...
def app():
    ...
    st.header('📊 All Programs Results Compilation')
    st.info("📋 Upload CSV from ERP and get consolidated results for all programs: Pass, Promoted with backlogs, and Withheld students")
    ...
    uploaded_file = st.file_uploader(
        ...
        type=['csv'],
        help="Upload the result CSV file from ERP system (should contain program information)"
    )
    ...
    has_proper_headers = any(col in ['Student ID', 'Student Name', 'Maximum Marks', 'Obtained Marks', 'Status', 'Course Variant', 'Program'] for col in result.columns)
    ...
    if all(str(i) == str(col) for i, col in enumerate(result.columns)) or not has_proper_headers:
        ...
        manual_mapping = {
            ...
            '1': 'Program'  # Assuming program is in column 1, adjust as needed
        }
        ...
        try:
            st.dataframe(result[['Student ID', 'Student Name', 'Maximum Marks', 'Obtained Marks', 'Status', 'Course Variant', 'Program']].head())
        except KeyError as e:
            st.error(f"❌ Error displaying mapped data: {e}")
            st.write("Available columns:", list(result.columns))
        
        # Verify all required columns are present
        required_columns = ['Student ID', 'Student Name', 'Maximum Marks', 'Obtained Marks', 'Status', 'Course Variant', 'Program']
        ...
        required_columns = ['Course Variant', 'Student ID', 'Student Name', 'Maximum Marks', 'Obtained Marks', 'Status', 'Program']
    ...
    essential_columns = ['Student ID', 'Student Name', 'Maximum Marks', 'Obtained Marks', 'Status', 'course code', 'Program']
    ...
    unique_programs = result['Program'].dropna().unique()
    st.write(f"**📚 Found {len(unique_programs)} unique programs:**")
    ...
    for program in unique_programs:
        st.markdown(f"---")
        st.subheader(f"📊 Processing Program: {program}")
        
        # Filter data for this program
        program_data = result[result['Program'] == program].copy()
        st.write(f"**Students in {program}: {len(program_data)}**")
        
        # Process this program's data
        try:
            program_data['Maximum Marks'] = pd.to_numeric(program_data['Maximum Marks'], errors='coerce')
            program_data['Obtained Marks'] = pd.to_numeric(program_data['Obtained Marks'], errors='coerce')
            
            result_obtained_marks = program_data.groupby('Student ID').agg({'Obtained Marks': [lambda x: x.iloc[0], 'sum']})
            
        except Exception as e:
            st.error(f"❌ Error in data processing for {program}: {str(e)}")
            continue
        ...
        try:
            ...
            st.error(f"❌ Error processing student marks for {program}: {str(e)}")
            continue
        
        # Total marks of the examination
        total_marks_series = program_data.groupby('Student ID')['Maximum Marks'].sum()
        if len(total_marks_series) == 0:
            st.error(f"❌ No student data found for total marks calculation in {program}")
            continue
        
        total_marks = total_marks_series.iloc[0]
        if total_marks == 0:
            st.error(f"❌ Total marks is zero - cannot calculate CGPA for {program}")
            continue
        ...
        try:
            final_result_with_cgpa = pd.merge(program_data,final_result_obtained_marks, how='right', on='Student ID')
            
            # CGPA is being dropped for the time being
            final_result_with_cgpa.drop('CGPA', axis=1, inplace=True)
        except Exception as e:
            st.error(f"❌ Error merging student data for {program}: {str(e)}")
            continue
        ...
        all_program_results[program] = {
            "total students appeared": total_students_appeared,
            "students passed": no_students_passed,
            "students failed": no_students_failed,
            "students witheld": no_students_witheld,
            "pass percent": np.round(no_students_passed/total_students_appeared*100, 2) if total_students_appeared > 0 else 0,
            "final_passed_student": final_passed_student,
            "fail_students_with_subject": fail_students_with_subject,
            "witheld_student": witheld_student
        }
        
        # Display summary for this program
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            st.metric("Total Appeared", total_students_appeared)
        with col2:
            st.metric("All Cleared", no_students_passed, f"{all_program_results[program]['pass percent']}%")
        with col3:
            st.metric("With Backlogs", no_students_failed)
        with col4:
            st.metric("Withheld", no_students_witheld)
    
    # Generate PDF for all programs
    if all_program_results:
        st.markdown("---")
        st.subheader("📄 Generate Individual Program Reports")
        
        if st.button("🚀 Generate Individual PDF Reports (ZIP)", type="primary"):
            import zipfile
            import tempfile
            
            # This section generates individual PDFs for each program
            def create_pdf_for_program(program, program_data, session, year, annual_semester, type, semester, date_str):
                # Create PDF buffer
                buffer = io.BytesIO()
                ...
                # Create info dictionary for this program
                info = {
                    "session": session,
                    "year": year,
                    "program": program,
                    ...
                    "total students appeared": program_data["total students appeared"],
                    "students passed": program_data["students passed"],
                    "students failed": program_data["students failed"],
                    "students witheld": program_data["students witheld"],
                    "pass percent": program_data["pass percent"]
                }
                ...
                last_page_template = PageTemplate(
                    id='LastPage',
                    frames=[frame_later],
                    onPageEnd=lambda c, d: draw_footer(c, d, date_str)
                )
                ...
                # Add tables for this program
                if len(program_data["final_passed_student"]) > 0:
                    elements.append(NextPageTemplate('FirstPage'))
                    elements.extend(create_students_table(program_data["final_passed_student"], "The following candidate(s) has/have cleared all the course(s)", table_style))
                    elements.append(Spacer(1, 10))
                
                if len(program_data["fail_students_with_subject"]) > 0:
                    if len(program_data["final_passed_student"]) > 0:
                        elements.append(NextPageTemplate('MiddlePages'))
                    else:
                        elements.append(NextPageTemplate('FirstPage'))
                    elements.extend(create_failed_students_table(program_data["fail_students_with_subject"], table_style))
                    elements.append(Spacer(1, 20))
                
                if len(program_data["witheld_student"]) > 0:
                    if len(program_data["final_passed_student"]) > 0 or len(program_data["fail_students_with_subject"]) > 0:
                        elements.append(NextPageTemplate('LastPage'))
                    else:
                        elements.append(NextPageTemplate('FirstPage'))
                    elements.extend(create_students_table(program_data["witheld_student"], "The following candidate(s)'s result has been withheld:", table_style))
                    elements.append(Spacer(1, 20))
                ...
                try:
                    ...
                    st.error(f"❌ PDF build failed for {program}: {e}")
                    return None
            ...
            # Generate individual PDFs and create ZIP file
            try:
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    total_programs = len(all_program_results)
                    
                    # Generate PDF for each program
                    for i, (program, program_data) in enumerate(all_program_results.items()):
                        st.write(f"📄 Generating PDF for: {program}")
                        
                        pdf_buffer = create_pdf_for_program(program, program_data, session, year, annual_semester, type, semester, date_str)
                        
                        if pdf_buffer is not None:
                            # Save PDF to temporary file
                            safe_program_name = "".join(c for c in program if c.isalnum() or c in (' ', '-', '_')).rstrip()
                            pdf_filename = f"{safe_program_name}_{year}_{session}.pdf"
                            ...
                            st.error(f"❌ Failed to generate PDF for: {program}")
                    ...
                    if generated_pdfs:
                        ...
                        # Download button
                        st.success(f"✅ Generated {len(generated_pdfs)} individual program reports!")
                        st.download_button(
                            label="📥 Download Individual Program Reports (ZIP)",
                            data=zip_buffer.getvalue(),
                            file_name=f'individual_program_reports_{year}_{session}.zip',
        ...
        st.warning("⚠️ No program data found to process.")
